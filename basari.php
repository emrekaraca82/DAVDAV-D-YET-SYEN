...
<div style="margin-top:15px; margin-left:10px;" class="container-fluid">
  <div class="row content">
    ...
    <div class="col-sm-9">
		...
		<div class="cizgi">
			<p>Sitenizdeki basari öykülerin buradan yönetebilirsiniz.</p>
		</div><br>
		<a href="basariadd.php" type="button" class="btn btn-success">Ekle</a><br><br>
...
<?php
        $sorgu = $baglanti->query("SELECT * FROM basari order by basari_id desc limit " . (($page - 1) * $sayfaBasinaKayitSayisi) . "," . $sayfaBasinaKayitSayisi);
        ...
        while ($sonuc = $sorgu->fetch_assoc()) { // verileri döngü ile sayfamıza yazıyoruz
            ?>					
					<tr>
						<td> <img  src="<?php echo $sonuc["basari_resim"];?>" class="card-img-top" width="100px;" height="50px;" ></td>						
						<td><?php echo $sonuc['basari_adi'] ?></td>
						
						<td><?php echo $sonuc['basari_tarih'] ?></td>
					    <td><?php echo "<a href='basariupdate.php?id=".$sonuc["basari_id"]." '>";?>
							<p class="btn btn-success">Düzenle</p></a>
						</td>
						<td><?php echo "<a href='basaridelete.php?id=".$sonuc["basari_id"]."' onclick='return confirmDel();'>";?>
